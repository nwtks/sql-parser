...
module SchemaParser =
    ...
    // 11.51 (<partial method specification>), all defined in this module. INSTANCE and
    // CONSTRUCTOR are not reserved words, hence the `attempt`s.
    let private pMethodKind =
        choice
    ...
    let pRoutineType =
        choice
            [ pKeyword "ROUTINE" >>% RoutineType.Routine
              ...
              // 10.6 pMethodKind also serves 11.51 / 11.60 (defined above).
              attempt (
    ...
    let private pConstraintEnforcement =
    ...
    let private pDefaultClause =
        ...
        pKeyword "DEFAULT" >>. pDefaultOption
    ...
    let private pBasicSequenceGeneratorOption =
        choice
    ...
    let private pSequenceGeneratorStartWithOption =
    ...
    let private pIdentityColumnSpecification =
    ...
    let private pSystemTimePeriodColumnSpecification =
        pKeyword "GENERATED"
        >>. pKeyword "ALWAYS"
        >>. pKeyword "AS"
        >>. pKeyword "ROW"
    ...
    let private pTimePeriodSpecification =
        attempt (
    ...
    let private pTablePeriodDefinition =
    ...
    let private pSelfReferencingColumn =
    ...
    let private pAlterSequenceGeneratorRestartOption =
    ...
    // 11.49 <triggered SQL statement> / 13.4 <SQL procedure statement> — the shared top-level
    ...
    let private pAttributeDefinition =
        pIdentifierExpression
        .>>. pDataType
        .>>. opt pDefaultClause
        .>>. opt (pKeyword "COLLATE" >>. pSchemaQualifiedNameExpression)
    ...
    let private pLocatorIndication = pKeyword "AS" >>. pKeyword "LOCATOR" >>% true

    // 11.60 <SQL parameter declaration list> ::= ( [ <SQL parameter declaration> [ { , <SQL parameter declaration> }... ] ] )
    let private pParameterDeclarationList =
    ...
    let private pParameterStyleClause =
        pKeyword "PARAMETER"
        >>. pKeyword "STYLE"
    ...
    // NOTE: pMethodKind (10.6 / 11.51 / 11.60) is defined above in this module, so the routine
    // and type parsers can share it.
    let private pPartialMethodSpecification =
        opt pMethodKind
    ...
    let private pOriginalMethodSpecification =
    ...
    let private pRoutineCharacteristics =
    ...
    let private pTransformElementList =
    ...
    let pAlterTransformStatement =
        ...
        let pAlterTransformGroup =
            ...
            .>>. between
                (token (pstring "("))
                (token (pstring ")"))
                (sepBy1 pAlterTransformAction (token (pstring ",")))
